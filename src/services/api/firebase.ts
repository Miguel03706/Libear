...
export default {
  createAccountFB: async (email: string, password: string) => {
  ...
  getOrientation: async (id: string) => {
    // const auth = getAuth()
    const ref = collection(db, 'ActivityRef')
  ...
  completeMission: async (missionId: number) => {
    ...
    //   if (id === null) return
    //   const missionRef = doc(db, 'userRef', id?.toString())
  ...
  buyItens: async (id: string, price: number) => {
